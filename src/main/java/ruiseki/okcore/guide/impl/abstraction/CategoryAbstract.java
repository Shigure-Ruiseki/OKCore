...
public abstract class CategoryAbstract {
     ...
     * Adds an entry to this category.
     * <p>
     * Shorthand of {@link #addEntry(ResourceLocation, EntryAbstract)}. Requires {@link #withKeyBase(String)} to have
     * been called.
     *
     * @param key   - The key of the entry to add.
     * @param entry - The entry to add.
    ...
    public void removeEntries(List<ResourceLocation> keys) {
        for (ResourceLocation key : keys) if (entries.containsKey(key)) entries.remove(key);
    }
