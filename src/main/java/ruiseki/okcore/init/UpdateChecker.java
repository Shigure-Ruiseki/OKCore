                        ...
                        if (element != null && element.isJsonObject()) {
                            ...
                            if (json.has("version")) {
                                ...
                                if (VersionHelpers.STATUS_OUTDATED.equals(status)) {
                                    mod.log(Level.WARN, "An update is available! Current: %s, Latest: %s", currentVersion, latestVersion);
                                } else {
                                    mod.log(Level.INFO, "Mod is up to date (%s).", currentVersion);
                                }
                            }

                            if (json.has("download") && json.get("download").isJsonObject()) {
                                ...
                                for (Map.Entry<String, JsonElement> entry : downloadObj.entrySet()) {
                                    if (entry.getValue().isJsonPrimitive()) {
                                        downloadMap.put(entry.getKey(), entry.getValue().getAsString());
                                    }
                                }
