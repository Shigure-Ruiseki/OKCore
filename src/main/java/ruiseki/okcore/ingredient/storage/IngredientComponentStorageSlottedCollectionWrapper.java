    ...
    public T insert(int slot, @Nonnull T ingredient, boolean simulate) {
        ...
        if (!matcher.isEmpty(insertingIngredient)) {
            ...
            if (matcher.isEmpty(contained)
                ...
                long addQuantity = Math.min(
                    getMaxQuantity(slot) - matcher.getQuantity(contained),
                    matcher.getQuantity(insertingIngredient));
                if (!simulate) {
                    ingredientCollection
                        .set(slot, matcher.withQuantity(ingredient, matcher.getQuantity(contained) + addQuantity));
    ...
    public T extract(int slot, long maxQuantity, boolean simulate) {
        ...
        if (!matcher.isEmpty(contained)) {
            ...
            if (!simulate) {
                ...
                ingredientCollection
                    .set(slot, matcher.withQuantity(contained, matcher.getQuantity(contained) - removeQuantity));
                ...
                if (matcher.matches(prototype, extractingIngredient, matchCondition)) {
                    if (!simulate) {
                        ...
                        ingredientCollection.set(
                            slot,
                            matcher.withQuantity(contained, matcher.getQuantity(contained) - removeQuantity));
                        this.quantity -= removeQuantity;
                    }
                    return extractingIngredient;
                }
