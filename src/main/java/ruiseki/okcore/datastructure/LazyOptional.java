 ...
 * {@link #addListener(NotNullConsumer)}. This method is invoked when the provider of
 * this object calls {@link #invalidate()}.
 * <p>
 * To create an instance of this class, use {@link #of(NotNullSupplier)}. Note
 * that this accepts a {@link NotNullSupplier}, so the result of the supplier
 * must never be null.
 * <p>
...
public class LazyOptional<T> {
     ...
     * {@link NotNullSupplier}.
     *
     * @param instanceSupplier The {@link NotNullSupplier} to wrap. Cannot return
     *                         null, but can be null itself. If null, this method
     *                         returns {@link #empty()}.
     */
    public static <T> LazyOptional<T> of(final @Nullable NotNullSupplier<T> instanceSupplier) {
    ...
    private LazyOptional(@Nullable NotNullSupplier<T> instanceSupplier) {
    ...
    /**
     * If non-empty, invoke the specified {@link NotNullConsumer} with the object,
     * otherwise do nothing.
     *
     * @param consumer The {@link NotNullConsumer} to run if this optional is non-empty.
     * @throws NullPointerException if {@code consumer} is null and this {@link LazyOptional} is non-empty
     */
    public void ifPresent(NotNullConsumer<? super T> consumer) {
     ...
     */
    public <U> LazyOptional<U> lazyMap(NotNullFunction<? super T, ? extends U> mapper) {
     ...
     *          For a non-resolving mapper that will lazily run the mapping, use {@link #lazyMap(NotNullFunction)}.
     *
     * @param mapper A mapping function to apply to the mod object, if present
     ...
     * @throws NullPointerException if {@code mapper} is null.
     */
    public <U> Optional<U> map(NotNullFunction<? super T, ? extends U> mapper) {
        Objects.requireNonNull(mapper);
     ...
     * {@link NotNullPredicate}, returning empty if false.
     * <p>
     ...
     * predicate.</em>
     *
     * @param predicate A {@link NotNullPredicate} to apply to the result of the
     *                  contained supplier, if non-empty
     * @return An {@link Optional} containing the result of the contained
     *         supplier, if and only if the passed {@link NotNullPredicate} returns
     ...
     */
    public Optional<T> filter(NotNullPredicate<? super T> predicate) {
        Objects.requireNonNull(predicate);
        final T value = getValue(); // To keep the non-null contract we have to evaluate right now. Should we allow this
                                    // function at all?
     ...
     *
     * @param other A {@link NotNullSupplier} whose result is returned if this
     ...
     */
    public T orElseGet(NotNullSupplier<? extends T> other) {
        T val = getValue();
     ...
     * exception created by the provided {@link NotNullSupplier}.
     *
     ...
     * @param exceptionSupplier The {@link NotNullSupplier} which will return the
     ...
     *                              {@link LazyOptional} is empty
     */
    public <X extends Throwable> T orElseThrow(NotNullSupplier<? extends X> exceptionSupplier) throws X {
        T val = getValue();
    ...
    /**
     * Register a {@link NotNullConsumer listener} that will be called when this {@link LazyOptional} becomes invalid
     * (via {@link #invalidate()}).
     * <p>
     * If this {@link LazyOptional} is empty, the listener will be called immediately.
     */
    public void addListener(NotNullConsumer<LazyOptional<T>> listener) {
        if (isPresent()) this.listeners.add(listener);
        else listener.accept(this);
    }

    /**
     * Unregisters a {@link NotNullConsumer listener} from the list to be notified when this {@link LazyOptional}
     * becomes invalid (via {@link #invalidate()}).<br>
     * This allows modder who know they will not need to be notified, to remove the hard reference that this holds to
     * their listener.
     */
    public void removeListener(NotNullConsumer<LazyOptional<T>> listener) {
        this.listeners.remove(listener);
    }

    /**
     * Invalidate this {@link LazyOptional}, making it unavailable for further use,
     * and notifying any {@link #addListener(NotNullConsumer) listeners} that this
