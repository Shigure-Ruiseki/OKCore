...
/// A [IImmutableItemStack] implementation that's meant to be passed to the various insert methods.
/// @see IInventoryIterator#insert(IImmutableItemStack, boolean)
/// @see IItemSink#store(IImmutableItemStack)
public class InsertionItemStack implements IImmutableItemStack {

    private boolean useImmutable = false;
    private ItemStack stack;
    private IImmutableItemStack immutable;
    @Nonnegative
    ...
    public InsertionItemStack(@NotNull IImmutableItemStack stack) {
        set(stack, stack.getStackSize());
    }

    public InsertionItemStack(@NotNull IImmutableItemStack stack, @Nonnegative int amountToInsert) {
        set(stack, amountToInsert);
    }
    ...
    public InsertionItemStack set(IImmutableItemStack stack) {
    ...
    public InsertionItemStack set(IImmutableItemStack stack, int amount) {
        this.useImmutable = true;
        this.immutable = Objects.requireNonNull(stack, "stack cannot be null");
        this.amountToInsert = Math.max(0, amount);
        return this;
    ...
    public int getItemMeta() {
        return useImmutable ? immutable.getItemMeta() : ItemStackHelpers.getStackMeta(stack);
    }
