...
public class IngredientMapMultiClassified<T, M, V> extends IngredientMapAdapter<T, M, V>
    ...
    public IngredientMapMultiClassified(IngredientComponent<T, M> component,
        ...
        for (IngredientComponentCategoryType<T, M, ?> categoryType : component.getCategoryTypes()) {
            this.classifiedMaps
                .put(categoryType, new IngredientMapSingleClassified<>(component, mapCreator, categoryType));
        }
    }
     ...
     */
    public int putAllThreaded(@Nullable ExecutorService executorService,
        IIngredientMap<? extends T, M, ? extends V> map) {
        ...
        try {
            int ret = executorService.invokeAll(
                this.classifiedMaps.values()
                    .stream()
                    .map(c -> (Callable<Integer>) () -> c.putAll(map))
    ...
    public Iterator<Map.Entry<T, V>> iterator() {
        return new IIngredientCollectionLikeMultiClassifiedTrait.RemoveCallbackIterator<>(
            this,
            null,
            getComponent().getMatcher()
                .getAnyMatchCondition());
