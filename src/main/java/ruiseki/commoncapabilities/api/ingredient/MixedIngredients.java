...
public class MixedIngredients extends MixedIngredientsAdapter {
    ...
    public static <T> MixedIngredients ofInstances(IngredientComponent<T, ?> component, Collection<T> instances) {
        ...
        ingredients
            .put(component, instances instanceof ArrayList ? (List<?>) instances : Lists.newArrayList(instances));
    ...
    public static MixedIngredients fromRecipeInput(IRecipeDefinition recipe) {
        ...
        for (IngredientComponent<?, ?> component : recipe.getInputComponents()) {
            ...
            List<?> instances = recipe.getInputs(component)
                .stream()
                .map(ingredient -> {
                    IPrototypedIngredient<?, ?> firstIngredient = Iterables
                        .getFirst(ingredient.getAlternatives(), null);
