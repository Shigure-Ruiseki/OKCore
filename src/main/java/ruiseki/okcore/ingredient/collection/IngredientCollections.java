...
public final class IngredientCollections {
    ...
    public static <T, M> boolean equalsCheckedOrdered(IIngredientCollection<T, M> c1, IIngredientCollection<T, M> c2) {
        ...
        IIngredientMatcher<T, M> matcher = c1.getComponent()
            .getMatcher();
        while (it1.hasNext()) {
    ...
    public static <T, M> int hash(IIngredientCollection<T, M> collection) {
        int hash = collection.getComponent()
            .hashCode();
        IIngredientMatcher<T, M> matcher = collection.getComponent()
            .getMatcher();
    ...
    public static <T, M, V> int hash(IIngredientMap<T, M, V> map) {
        int hash = map.getComponent()
            .hashCode();
        IIngredientMatcher<T, M> matcher = map.getComponent()
            .getMatcher();
        for (Map.Entry<T, V> entry : map.entrySet()) {
            hash = hash | (matcher.hash(entry.getKey()) ^ entry.getValue()
                .hashCode());
        }
        return hash;
    }
    ...
    public static <C extends IIngredientCollectionMutable<?, ?>> C deserialize(NBTTagCompound tag,
        IIngredientCollectionConstructor<C> ingredientCollectionFactory) {
        // Validate tag
        ...
        for (Object subTag : ingredients.tagList) {
            collectionUnsafe.add(serializer.deserializeInstance((NBTBase) subTag));
        }
