...
package ruiseki.okcore.client.render.block;
...
public final class WorldRender implements ISimpleBlockRenderingHandler {
    ...
    public void renderItemBlock(final ItemStack item, final IItemRenderer.ItemRenderType type, final Object[] data) {
        ...
        if (blk instanceof BlockOK block) {
            ...
            if (!this.hasError) {
                this.hasError = true;
                OKCore.okLog(Level.ERROR, "Invalid render - item/block mismatch");
                OKCore.okLog(Level.ERROR, "		item: " + item.getUnlocalizedName());
                OKCore.okLog(Level.ERROR, "		block: " + blk.getUnlocalizedName());
            }
        }
    }
}
