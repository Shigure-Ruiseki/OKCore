...
public class IngredientCollectionSingleClassifiedCollapsed<T, M, C>
     ...
     */
    public IngredientCollectionSingleClassifiedCollapsed(IngredientComponent<T, M> component,
    ...
    public int size() {
        return this.getClassifiedCollections()
            .values()
            .stream()
            .mapToInt(IIngredientCollectionLike::size)
            .sum();
    }

    @Override
    public long getQuantity(T instance) {
        IIngredientCollapsedCollectionMutable<T, M> collection = getClassifiedCollections()
