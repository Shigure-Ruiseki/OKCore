...
public class IngredientComponentStorageCollectionWrapper<T, M> implements IIngredientComponentStorage<T, M> {
    ...
    public IngredientComponentStorageCollectionWrapper(
        IIngredientCollapsedCollectionMutable<T, M> ingredientCollection) {
        ...
        if (!matcher.isEmpty(insertingIngredient)) {
            ...
            if (added) {
                ...
                return matcher.withQuantity(
                    insertingIngredient,
                    matcher.getQuantity(ingredient) - matcher.getQuantity(insertingIngredient));
            }
        }
        return ingredient;
    }
    ...
    public T extract(@Nonnull T prototype, M matchCondition, boolean simulate) {
        ...
        Iterator<T> it = this.ingredientCollection.iterator(
            prototype,
    ...
    public T extract(long maxQuantity, boolean simulate) {
        ...
        if (!simulate) {
            this.ingredientCollection.remove(toExtract);
            this.quantity -= getComponent().getMatcher()
                .getQuantity(toExtract);
        }
