...
public class PrototypedIngredient<T, M> implements IPrototypedIngredient<T, M> {
    ...
    public boolean equals(Object obj) {
        if (obj instanceof IPrototypedIngredient) {
            ...
            return this.getComponent()
                .equals(that.getComponent())
                && this.getComponent()
                    .getMatcher()
                    .matchesExactly(this.getPrototype(), (T) that.getPrototype())
    ...
    @Override
    public int hashCode() {
        return 394 | this.getComponent()
            .hashCode() << 4
            | this.getComponent()
                .getMatcher()
                .hash(this.getPrototype()) << 2
            | this.getCondition()
                .hashCode();
    }
    ...
    public int compareTo(IPrototypedIngredient<?, ?> that) {
        int compComp = this.getComponent()
            .compareTo(that.getComponent());
        if (compComp != 0) {
            return compComp;
        }

        IIngredientMatcher<T, M> matcher = this.getComponent()
            .getMatcher();
