...
public class IngredientCollectionPrototypeMap<T, M> extends IngredientCollectionAdapter<T, M>
    ...
    public IngredientCollectionPrototypeMap(IngredientComponent<T, M> component, boolean negativeQuantities,
        ...
        if (getComponent().getPrimaryQuantifier() == null) {
            throw new IllegalArgumentException(
                "Quantitative grouping requires a primary quantifier on the component type.");
        }
    }
    ...
    public boolean contains(T instance, M matchCondition) {
        ...
        if (matcher.getExactMatchCondition()
        ...
        boolean exactMatch = getComponent().getPrimaryQuantifier() != null && matcher.hasCondition(
            matchCondition,
    ...
    public Iterator<T> iterator(T instance, M matchCondition) {
        ...
        if (!matcher.hasCondition(
            ...
            getComponent().getPrimaryQuantifier()
                .getMatchCondition())) {
            return Iterators
                .transform(ingredients.iterator(instance, matchCondition), new QuantityApplier<>(getComponent()));
        }
