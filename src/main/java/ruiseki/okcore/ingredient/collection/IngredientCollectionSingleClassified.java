...
public class IngredientCollectionSingleClassified<T, M, C, L extends IIngredientCollectionMutable<T, M>>
    extends IngredientCollectionAdapter<T, M> implements IIngredientCollectionLikeSingleClassifiedTrait<T, M, T, C, L> {
     ...
     */
    public IngredientCollectionSingleClassified(IngredientComponent<T, M> component, Supplier<L> collectionCreator,
        ...
        if (appliesToClassifier(matchCondition)) {
            ...
            if (collection != null) {
                if (Objects.equals(getCategoryType().getMatchCondition(), matchCondition)) {
                    ...
                    M subMatchCondition = getComponent().getMatcher()
                        .withoutCondition(matchCondition, getCategoryType().getMatchCondition());
    ...
    public boolean contains(T instance, M matchCondition) {
        ...
        return this.classifiedCollections.values()
            .stream()
            .anyMatch(c -> c.contains(instance, matchCondition));
    }
    ...
    public Iterator<T> iterator(T instance, M matchCondition) {
        if (getComponent().getMatcher()
            .getAnyMatchCondition()
            .equals(matchCondition)) {
            return iterator();
        }
        if (appliesToClassifier(matchCondition)) {
            // At most one classifier will be found, so we can simply delegate this call
            L collection = this.getClassifiedCollections()
                .get(getClassifier(instance));
            if (collection != null) {
        ...
        return new FilteredIngredientCollectionLikeSingleClassifiedIterator<>(
            this,
            getComponent().getMatcher(),
            instance,
            matchCondition);
