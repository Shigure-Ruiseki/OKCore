package ruiseki.okcore.ingredient.storage;
...
public final class IngredientStorageHelpers {
    ...
    public static <T, M> T moveIngredientsIterative(IIngredientComponentStorage<T, M> source,
        IIngredientComponentStorage<T, M> destination, long maxQuantity, boolean simulate)
     ...
     */
    public static <T, M> T moveIngredientsIterative(IIngredientComponentStorage<T, M> source,
        IIngredientComponentStorage<T, M> destination, T instance, M matchCondition, boolean simulate)
        throws InconsistentIngredientInsertionException {
        IngredientComponent<T, M> component = source.getComponent();
        IIngredientMatcher<T, M> matcher = component.getMatcher();
     ...
     */
    public static <T, M> T moveIngredients(IIngredientComponentStorage<T, M> source,
        IIngredientComponentStorage<T, M> destination, Predicate<T> predicate, long maxQuantity, boolean exactQuantity,
        boolean simulate) throws InconsistentIngredientInsertionException {
        IIngredientMatcher<T, M> matcher = source.getComponent()
            .getMatcher();
        for (T extractedSimulated : source) {
            if (predicate.test(extractedSimulated)) {
                ...
                extractedSimulated = source
                    .extract(extractedSimulated, matcher.getExactMatchNoQuantityCondition(), true);
                if (!matcher.isEmpty(extractedSimulated)) {
                    T movable = insertIngredient(destination, extractedSimulated, true);
                    if (!matcher.isEmpty(movable) && (exactQuantity ? matcher.getQuantity(movable) == maxQuantity
    ...
    public static <T, M> T moveIngredientsSlotted(IIngredientComponentStorage<T, M> source, int sourceSlot,
        ...
        if (!loopSourceSlots && !loopDestinationSlots) {
            ...
            if (!matcher.isEmpty(extractedSimulated) && matcher.matches(instance, extractedSimulated, matchCondition)) {
                ...
                if (remainingQuantity == 0 || (remainingQuantity < prototypeQuantity && !matcher.hasCondition(
                    matchCondition,
                    source.getComponent()
                        .getPrimaryQuantifier()
                        .getMatchCondition()))) {
                    return moveEffectiveSourceExactDestinationExact(
                        sourceSlotted,
                        sourceSlot,
                        destinationSlotted,
                        destinationSlot,
            ...
            if (source instanceof IIngredientComponentStorageSlotted) {
                ...
                for (int slot = 0; slot < slots; slot++) {
                    T moved = moveIngredientsSlotted(
                        source,
                        slot,
                        destination,
                        destinationSlot,
                        ...
                        if (matcher.matches(
                            ...
                            if (!matcher.isEmpty(extractedSimulated)) {
                                ...
                                if (remainingQuantity == 0
                                    || (remainingQuantity < prototypeQuantity && !matcher.hasCondition(
                                        matchCondition,
                                        source.getComponent()
                                            .getPrimaryQuantifier()
                                            .getMatchCondition()))) {
                                    T moved = moveEffectiveSourceNonSlottedDestinationExact(
                                        source,
                                        destinationSlotted,
                                        destinationSlot,
                                        matchCondition,
            ...
            if (destination instanceof IIngredientComponentStorageSlotted) {
                ...
                if (!matcher.isEmpty(sourceInstance) && matcher.matches(instance, sourceInstance, matchCondition)) {
                    ...
                    return moveEffectiveSourceExactDestinationNonSlotted(
                        sourceSlotted,
                        sourceSlot,
                        destination,
                        inserted,
     ...
     */
    public static <T, M> T moveIngredientsSlotted(IIngredientComponentStorage<T, M> source, int sourceSlot,
        IIngredientComponentStorage<T, M> destination, int destinationSlot, Predicate<T> predicate, long maxQuantity,
        boolean exactQuantity, boolean simulate) throws InconsistentIngredientInsertionException {
        IIngredientMatcher<T, M> matcher = source.getComponent()
            .getMatcher();
        boolean loopSourceSlots = sourceSlot < 0;
        boolean loopDestinationSlots = destinationSlot < 0;
                ...
                if (loopDestinationSlots) {
                    ...
                    for (T sourceInstance : source) {
                        if (predicate.test(sourceInstance)) {
                            ...
                            T extractedSimulated = source
                                .extract(sourceInstance, matcher.getExactMatchCondition(), true);
                            if (!matcher.isEmpty(extractedSimulated)) {
                                T remaining = destinationSlotted.insert(destinationSlot, extractedSimulated, true);
                                long remainingQuantity = matcher.getQuantity(remaining);
                                if (remainingQuantity == 0 || (remainingQuantity < maxQuantity && !exactQuantity)) {
    ...
    protected static <T, M> T moveEffectiveSourceExactDestinationExact(
        IIngredientComponentStorageSlotted<T, M> sourceSlotted, int sourceSlot,
        IIngredientComponentStorageSlotted<T, M> destinationSlotted, int destinationSlot, T extractedSimulated,
        T remaining, boolean simulate) throws InconsistentIngredientInsertionException {
        IIngredientMatcher<T, M> matcher = sourceSlotted.getComponent()
            .getMatcher();
        if (simulate) {
            ...
            if (!matcher.isEmpty(sourceInstanceEffective)) {
                ...
                if (matcher.isEmpty(remainingEffective)) {
                    ...
                    if (!matcher.isEmpty(remainderFixup)) {
                        T movedActual = matcher.withQuantity(
                            remainingEffective,
                            matcher.getQuantity(sourceInstanceEffective) - matcher.getQuantity(remainingEffective)
                                + matcher.getQuantity(remainderFixup));
                        throw new InconsistentIngredientInsertionException(
                            destinationSlotted.getComponent(),
                            destinationSlotted,
                            remainderFixup,
                            movedActual);
                    }
    ...
    protected static <T, M> T moveEffectiveSourceExactDestinationNonSlotted(
        IIngredientComponentStorageSlotted<T, M> sourceSlotted, int sourceSlot,
        IIngredientComponentStorage<T, M> destination, T inserted, boolean simulate)
        throws InconsistentIngredientInsertionException {
        IIngredientMatcher<T, M> matcher = sourceSlotted.getComponent()
            .getMatcher();
        if (simulate) {
    ...
    protected static <T, M> T moveEffectiveSourceNonSlottedDestinationExact(IIngredientComponentStorage<T, M> source,
        IIngredientComponentStorageSlotted<T, M> destinationSlotted, int destinationSlot, M extractMatchCondition,
        T extractedSimulated, T remaining, boolean simulate) throws InconsistentIngredientInsertionException {
        IIngredientMatcher<T, M> matcher = source.getComponent()
            .getMatcher();
        ...
        if (matcher.isEmpty(remaining)) {
            ...
            shouldMove = matcher.withQuantity(
                extractedSimulated,
                matcher.getQuantity(extractedSimulated) - matcher.getQuantity(remaining));
        }
    ...
    public static <T, M> T handleRemainder(IIngredientComponentStorage<T, M> source,
        IIngredientComponentStorage<T, M> destination, T movedEffective, T remainingEffective)
        throws InconsistentIngredientInsertionException {
        IIngredientMatcher<T, M> matcher = source.getComponent()
            .getMatcher();
        ...
        if (remainingEffectiveEmpty) {
            ...
            if (!matcher.isEmpty(remainderFixup)) {
                T movedActual = matcher.withQuantity(
                    remainingEffective,
                    matcher.getQuantity(remainingEffective) - matcher.getQuantity(remainingEffective)
                        + matcher.getQuantity(remainderFixup));
                throw new InconsistentIngredientInsertionException(
            ...
            return matcher.withQuantity(
                remainingEffective,
                matcher.getQuantity(movedEffective) - matcher.getQuantity(remainingEffective));
        }
    }
        ...
        if (!matcher.isEmpty(extractedSimulated)) {
            ...
            if (movableQuantity > 0) {
                if (simulate) {
                    ...
                    if (matcher.getQuantity(instance) != movableQuantity && matcher.hasCondition(
                        matchCondition,
                        source.getComponent()
                            .getPrimaryQuantifier()
                            .getMatchCondition())) {
                    ...
                    T extracted = source
                        .extract(matcher.withQuantity(extractedSimulated, movableQuantity), matchCondition, false);
                    return insertIngredientRemainderFixup(source, destination, extracted, false);
                }
            }
        }
     ...
     */
    public static <T, M> long insertIngredientQuantity(IIngredientComponentStorage<T, M> destination, T instance,
        boolean simulate) {
        IIngredientMatcher<T, M> matcher = destination.getComponent()
            .getMatcher();
     ...
     */
    public static <T, M> T insertIngredient(IIngredientComponentStorage<T, M> destination, T instance,
        boolean simulate) {
        IIngredientMatcher<T, M> matcher = destination.getComponent()
            .getMatcher();
    ...
    public static <T, M> T insertIngredientRemainderFixup(IIngredientComponentStorage<T, M> source,
        IIngredientComponentStorage<T, M> destination, T instance, boolean simulate)
        throws InconsistentIngredientInsertionException {
        if (simulate) {
     ...
     */
    public static <T, M> IIngredientComponentStorage<T, M> wrapStorage(
        IIngredientComponentStorage<T, M> ingredientComponentStorage, boolean read, boolean insert, boolean extract) {
        if (ingredientComponentStorage instanceof IIngredientComponentStorageSlotted) {
            return new IngredientComponentStorageSlottedWrapped<>(
                (IIngredientComponentStorageSlotted<T, M>) ingredientComponentStorage,
                read,
                insert,
                extract);
        } else {
    ...
    public static <T, M> NBTTagCompound serialize(IIngredientComponentStorage<T, M> storage) {
        NBTTagCompound tag = IngredientCollections
            .serialize(new IngredientArrayList<>(storage.getComponent(), storage.iterator()));
    ...
    public static IIngredientComponentStorage<?, ?> deserialize(NBTTagCompound tag, long rateLimit) {
        ...
        if (tag.getBoolean("slotted")) {
            return new IngredientComponentStorageSlottedCollectionWrapper<>(
                IngredientCollections.deserialize(tag, IngredientArrayList::new),
                maxQuantity,
                rateLimit);
        } else {
    ...
    /**
     * Helper interface for constructing an {@link ruiseki.okcore.ingredient.collection.IIngredientCollection} based on
     * an {@link IngredientComponent}.
     * 
